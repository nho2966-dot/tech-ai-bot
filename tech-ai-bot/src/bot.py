...
import tweepy
import random
import google.generativeai as genai  # ✅ تم الإصلاح
import cohere  # ✅ تم الإصلاح
...
def generate_tech_content():
    ...
    try:
        ...
        if not tavily_key or not cohere_key:
            raise ValueError("TAVILY_KEY or COHERE_API_KEY environment variables not set.")

        # --- STEP 1: Get raw content using Tavily (Data Source) ---
        response = requests.post(
            "https://api.tavily.com/search",  # ✅ تم الإصلاح
            json={
                "api_key": tavily_key,
        ...
        co = cohere.Client(cohere_key)  # ✅ تم الإصلاح
        ...
        model = genai.GenerativeModel('gemini-2.5-flash')  # ✅ تم الإصلاح
        ...
        gemini_response = model.generate_content(gemini_prompt)  # ✅ تم الإصلاح
...
def run_mission():
    logging.info("🚀 بدء محاولة النشر النهائية باستخدام Gemini و Cohere...")
    try:
        ...
        tweet_text = f"🛡️ موثوق | {content[:content_max_len]}\n\n🔗 {source_url}"
        
        if len(tweet_text) > max_tweet_length:
            tweet_text = tweet_text[:max_tweet_length - len(source_url) - 5] + '... 🔗 ' + source_url
        ...
        if response.data:
            logging.info(f"✅ تم النشر بنجاح! Tweet ID: {response.data['id']}")
        else:
            logging.warning("⚠️ Script completed, but X did not confirm the post.")

    except Exception as e:
        logging.error(f"❌ A precise technical error occurred: {e}")
